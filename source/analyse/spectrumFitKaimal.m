function [jLiOpt, sigmaOpt, sigmaNoiseOpt] = spectrumFitKaimal(freq, siiData, uvwBar, freqLimits, includeNoise, varargin)
...
%
% Syntax:
%       [jLiData, sigmaData, jLiOpt, sigmaOpt, sigmaNoiseOpt] = spectrumFitVonKarman(dt, uvw)
%
...
raiseFigure('spectrumFitKaimal() - Iteration Progress')
clf
subplot(2,2,1)
iter = 0;
...
xlabel('Iteration #')
ylabel('Least Squares Error between Kaimal fit and modelled data')
grid on
...
function sqDiff = nestEvalSpectrumFit(xParams)
    ...
    [siiVK] = spectrumKaimal(jLi, uvwBar, freq, sigma);
...
function sqDiff = nestEvalSingleSpectrumFit(xParams)
    ...
    switch component
        ...
        case 2
            ...
            % Get the unnormalised autospectrum
            [siiVK] = spectrumKaimalc(jLi, uvwBar, freq, sigma);
