function [windows] = windowVector(tYD, uvw, heading, pitch, roll, pressure, varargin)
...
%       fields)
%
%       [windows] = windowVector(..., 'WaveletDenoise', tfDenoise) Denoises uvw using
%       a biorthogonal second order (2:2) spline wavelet function as discussed
%       in Chapter 6, Ref [1].
%       
%       [windows] = windowVector(..., 'WaveletLengthScaleFilter', lowerUpper) Removes
%       components of the flow whose scales are outside the bounds in
%       lowerUpper.
%
% Inputs:
%
...
%
%       pitch           [n x 1]     Pitch of the vector probe at time tYD
%
%       roll            [n x 1]     Roll of the vector probe at time tYD
%
...
% Outputs:
%
%       binned          [m x 1] structure
%                                   Structure for m bins containing the
%                                   following fields:
%
...
opts.WaveletDenoise = false; % No denoising
opts.WaveletLengthScaleFilter = [0 Inf]; % No scale length limitations
opts = parse_pv_pairs(opts,varargin);

% Initialise the structure for output
windows = struct('startTimeYD', [],  'time', [], 'uvw', [], 'heading', [],'pitch', [], 'roll',[], 'pressure', []);
...
for i = 1:numel(startTimes)
    [valid, window] = getWindow(startTimes(i), tYD, uvw, heading, pitch, roll, pressure);
    if valid
...
function [valid, window] = getWindow(startTimeYD, tYD, uvwFilt, heading, pitch, roll, pressure)
...
% try
    ...
    window.pitch = pitch(startInd:endInd);
    window.roll = roll(startInd:endInd);
